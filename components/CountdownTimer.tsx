...
export const CountdownTimer: React.FC = () => {
  ...
  const lastTrackedDayRef = useRef<number | null>(null);

  useEffect(() => {
    if (timeRemaining.totalMilliseconds > 0) {
      if (lastTrackedDayRef.current !== timeRemaining.nextDay) {
        analytics.trackCountdownVisible(timeRemaining.nextDay);
        lastTrackedDayRef.current = timeRemaining.nextDay;
      }
    }
  }, [timeRemaining.totalMilliseconds, timeRemaining.nextDay]);
    ...
    <div className="bg-gradient-to-r from-green-800/50 to-red-800/50 backdrop-blur-md rounded-2xl p-4 md:p-6 border-2 border-yellow-500/40 shadow-xl max-w-md mx-auto mb-6">
      <div className="text-center">
        ...
        <div className="flex flex-wrap justify-center gap-2 md:gap-4 mb-2">
          {/* Days */}
          {days > 0 && (
            <div className="flex flex-col items-center bg-black/30 rounded-lg p-2 md:p-3 min-w-[52px] sm:min-w-[60px] md:min-w-[80px]">
